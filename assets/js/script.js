...
const urlImgPokemonDetail =
  "https://assets.pokemon.com/assets/cms2/img/pokedex/detail/";
const urlImgPokemonFull =
...
function card(cardPokemon) {
  ...
  $(".header__logo, #navegation__back, #search__button-type").click(
    function () {
      $("input").val("");
    }
  );
  ...
  $(".main").append(
    `<div class="card col-sm-6 col-md-4 col-xl-3">
        <img src="${
          urlImgPokemonDetail + idPokemon
        }.png" class="card__img" alt="${namePokemon}">
...
function homePokemon(url) {
  ...
  // se hace la llamada inicial al API de pokemons para recibir los primeros 40 pokemons
  $.get(url, (data) => {
    Next = data.next;
...
$.get(urlTypePokemons, (type) => {
  type.results.forEach((typePokemon) => {
    $("#search__select").append(
      `<option class="search__option">${typePokemon.name}</option>`
    );
  });
});
